...
function ChapterReader({ book, chapter, onClose, onOpenSaying, onNavigate }: { book: Saying['book']; chapter: number; onClose: () => void; onOpenSaying: (saying: Saying) => void; onNavigate: (chapter: { book: Saying['book']; number: number }) => void }) {
  ...
  return <div className="chapter-backdrop"><article className="chapter-reader" role="dialog" aria-modal="true" aria-labelledby="chapter-title">
    ...
    <main className="chapter-content">{entries.map((item) => { const saying = speechByVerse.get(item.verse); return <article className={saying ? 'chapter-entry speech-verse' : 'chapter-entry'} key={`${item.book}-${item.chapter}-${item.verse}`}><span className="chapter-verse">{item.verse}</span><div>{saying && <button className="chapter-entry-title" onClick={() => onOpenSaying(saying)}>{saying.title} · Words of Yeshua <ArrowRight size={14} /></button>}<p>{item.text}</p></div></article> })}</main>
